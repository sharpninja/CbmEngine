...
## FR-CBM-HOST-007 Safe disposable FrameLease for hybrid pump framebuffer acquisition (review remediation)

Extend the host viewport/pump with a safe ref-struct FrameLease (internal) so callers can use 'using var lease = Acquire...' guaranteeing Monitor release even on exceptions during blit/upload. Removes reliance on manual ReleaseFrame. Addresses brittle lock pattern identified in code review.
Scope: layer-1+
...
## FR-CBM-ROM-001 C64 ROM base resolution + runtime download-on-demand

RomDiscovery resolves the C64 ROM base (CBMENGINE_ROM_BASE env override when it points at an existing directory, otherwise the per-user cache RomCache.DefaultBasePath under LocalApplicationData) and acquires any missing ROMs at runtime via the ViceSharp locator (download, SHA256 verify, cache), materializing the canonical VICE filename the C64 machine profile requires. No git submodule and no bundled or build-time ROMs. RomDiscovery.DiscoverOrDownloadAsync returns a ready IRomProvider; CommodoreSystem.Build(profileId) plus the tool and sample entry points delegate.
Scope: layer-1+
**Acceptance Criteria:**
- [x] CBMENGINE_ROM_BASE pointing at an existing directory wins (highest precedence).
- [x] With no env override, DiscoverRomBase returns the per-user cache (RomCache.DefaultBasePath under LocalApplicationData, ending CbmEngine/roms).
- [x] EnsureC64RomsAsync downloads basic/kernal/characters when absent and no-ops when present; RomProviderAcquirer materializes the canonical VICE filename (basic-901226-01.bin, kernal-901227-03.bin, chargen-901225-01.bin).
- [x] DiscoverRomBase does not throw when ROMs are absent; it returns the cache base for download-on-demand (required-ROM enforcement is the machine build, not discovery).
- [x] DiscoverOrDownloadAsync returns an IRomProvider whose IsAvailable is true for the required C64 ROMs after acquisition.
- [x] CommodoreSystem.Build(c64) boots from the acquired ROMs.
- [x] EncodeCbmVid, Game.Sample, and CbmVidStudio entry points resolve ROMs via RomDiscovery/StudioRoms download-on-demand; the sample still runs.

## FR-CBM-SAMPLE-001 Refactor sample to reduce duplication, use ILogger instead of Console.WriteLine, and encapsulate repeating test harness into engine feature (review remediation)

Refactor src/CbmEngine.Game.Sample/Program.cs (in scope per review feedback) to extract common cart-build/boot/headless logic. Replace Console.WriteLine with Microsoft.Extensions.Logging.ILogger. Encapsulate the repeated test harness code (CommodoreSystem.Build + marker boot + warmup) as a supported feature in the engine (e.g. in Systems.Testing or Boot). Sample refactor is in scope.
Scope: layer-1+
